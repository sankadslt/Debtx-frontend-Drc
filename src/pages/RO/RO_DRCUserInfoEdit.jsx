...
// --- API Function for user profile update
const updateUserProfile = async (userProfilePayload) => {
  const apiUrl = 'https://debtx.slt.lk:6500/users/update/profile';
  try {
...
export default function RO_DRCUserDetailsEdit() {
    ...
    if (digitsOnly.length > 10) {
      ...
      setContactNoError(
        digitsOnly.length > 0 && digitsOnly.length < 9
          ? 'Contact number must be 9-10 digits.'
          : ''
      );
      setContactNo(cleaned);
    }
  ...
  const validateInputs = () => {
    ...
    // Validate RTOM areas for RO
    if (itemType === 'RO' && rtomAreas.length > 0) {
      const invalidRtom = rtomAreas.find(
        area => !rtomAreaOptions.some(opt => opt.area_name === area.name)
      );
      if (invalidRtom) {
        Swal.fire({
          title: 'Invalid RTOM',
  ...
  //     if (response.success) {
  //       // ---- NEW: Call Profile API for RO ----
  //       if (itemType === 'RO' && roId && fetchedData) {
  //         const ro_name =
  //           fetchedData?.recovery_officer_name || fetchedData?.drcUser_name || '';
  //         const profilePayload = {
  //           user_id: roId,
  //           profile_payload: {
  //             username: ro_name,
  //             email: email,
  //             user_nic: fetchedData?.nic || '',
  //             user_designation: 'recovery_officer',
  //           }
  //         };
  //         try {
  //           await updateUserProfile(profilePayload);
  //           // No popup needed on success
  //         } catch (profileErr) {
  //           Swal.fire({
  //             title: 'Profile Update Warning',
  //             text: `User main data saved, but profile update failed: ${profileErr.message || profileErr}`,
  //             icon: 'warning',
  ...
  try {
    ...
    if (response.success) {
      // ---- NEW: Call Profile API for RO ONLY IF EMAIL WAS EDITED ----
      if (
        itemType === 'RO' &&
        roId &&
        fetchedData &&
        email !== initialEmail // <-- Only if email changed
      ) {
        const ro_name = fetchedData?.recovery_officer_name || fetchedData?.drcUser_name || '';
        const profilePayload = {
          user_id: roId,
          profile_payload: {
            username: ro_name,
  ...
  const handleRemoveRtomArea = (index) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const newRtomAreas = rtomAreas.filter((_, i) => i !== index);
        if (
          drcUserStatus === 'Active' &&
          newRtomAreas.length > 0 &&
          !newRtomAreas.some(area => area.status)
        ) {
          Swal.fire({
            title: 'Cannot Remove RTOM',
            text: 'At least one RTOM area must remain active when the user status is Active.',
